  ...
  std::vector<render::anim::Animation>& animationsOut)
{
  ...
  for (int i = 0; i < model.nodes.size(); ++i) {
    ...
    if (node.mesh != -1) {
      ...
      if (!assetModel._meshes.empty()) {
        render::asset::Prefab prefab{};

        assetModel._id = util::Uuid::generate();
        prefab._model = assetModel._id;
        prefab._skeleton = skeleton._id ? skeleton._id : util::Uuid();
        prefab._name = assetModel._name;
        prefab._transform = m;

        for (auto& idx : materialIndices) {
          prefab._materials.push_back(materialsOut[idx]._id);
        }
