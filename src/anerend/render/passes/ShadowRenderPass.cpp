...
{
  registerDirectionalShadowPass(fgb, rc);
  registerTerrainDirectionalShadowPass(fgb, rc);
  registerPointShadowPass(fgb, rc);
}

void ShadowRenderPass::registerDirectionalShadowPass(FrameGraphBuilder& fgb, RenderContext* rc)
{
...
void ShadowRenderPass::registerTerrainDirectionalShadowPass(FrameGraphBuilder& fgb, RenderContext* rc)
{
  RenderPassRegisterInfo info{};
  info._name = "TerrainShadow";
  info._group = "Shadow";
  ...
  GraphicsPipelineCreateParams params{};
  params.device = rc->device();
  params.vertShader = "terrain_shadow_vert.spv";
  params.colorLoc = -1;
  params.normalLoc = -1;
  params.colorAttachment = false;
  params.depthBiasEnable = true;
  ...
  fgb.registerRenderPassExe("TerrainShadow",
    [this](RenderExeParams exeParams)
    {
      if (!exeParams.rc->getRenderOptions().directionalShadows) return;
