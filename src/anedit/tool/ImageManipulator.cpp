...
void ImageManipulator::paint8Bit(
  ...
  bool keepMax)
{
  if (x > texture._width - 1 || y > texture._height - 1) {
    return;
  }
  ...
  if (x >= _brush._radius && w - x > _brush._radius &&
    y >= _brush._radius && h - y > _brush._radius) {
  ...
  // TODO: Corner cases. Limit radius or something?
}
